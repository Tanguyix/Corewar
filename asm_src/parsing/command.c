...
static int	parse_cmd(t_prog *prog, unsigned int *i, unsigned int j)
{
	int	ret;
	...
	{
		if ((ret = parse_param(prog, i, j, CMD_COMMENT)))
			return (ret);
		return (0);
	}
	debug(ERROR, prog, CMD_UNKNOW, ((long)*i) << 32 | (j - 1));
	gotonxl(prog->content, i);
	return (SORC);
}

int			next_cmd(t_prog *prog, unsigned int *i)
{
	unsigned short	j;
	int				ret;
